    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker) {
        if (!attacker.worldObj.isRemote) {
            if (attacker instanceof EntityPlayer && target instanceof EntityMob) {
                if (target instanceof EntityZombie || target instanceof EntityZombie || target instanceof  EntitySkeleton || target instanceof  EntityWitch) {
                    ...
                    if (!(target instanceof EntityCreeper)){
                        new Timer().schedule(new TimerTask() {
                            @Override
                            public void run() {
                                if (mob!=null&&mob.isDead == false) mob.setHealth(0f);
                            }
                        }, 15 * 1000);
                    }
                }
